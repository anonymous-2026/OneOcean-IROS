...
def _pick_gebco_folder() -> Path:
    ...
    candidates = [
        _HERE / "Data" / "gebco_2025_sub_ice_topo_geotiff",
        _HERE / "Data" / "gebco_2024_sub_ice_topo_geotiff",
    ]
    for p in candidates:
...
folder_path = str(_pick_gebco_folder())
...
def Get_GeoTIFF_Data(lat_min, lat_max, lon_min, lon_max, elev_min, elev_max, save_path):
    # Try to infer the GEBCO year from the folder name for plot titles.
    gebco_year = "GEBCO"
    m = re.search(r"gebco_(\d{4})", folder_path)
    if m:
        gebco_year = f"GEBCO {m.group(1)}"
        ...
        if match:
            ...
            if (s <= lat_min <= n or s <= lat_max <= n) and (w <= lon_min <= e or w <= lon_max <= e):
                ...
                with rasterio.open(file_path) as dataset:
                    ...
                    plt.ylabel('Latitude')
                    plt.title(f'Underwater Terrain ({gebco_year}) ({lat_min}° to {lat_max}°N, {lon_min}° to {lon_max}°E, {elev_min}m to {elev_max}m)')
