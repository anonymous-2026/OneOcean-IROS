...
def render_scene(
    *,
    ...
    # External scene meshes (e.g., shipwrecks/structures).
    for mesh in meshes:
        ...
        for fi in midx.tolist():
            poly = np.round(face_uv[fi]).astype(np.int32)
            depth = float(face_depth[fi])
            fog_t = _underwater_fog_t(depth_m=depth, fog_start_m=6.0, fog_end_m=120.0)
            normal = (rot @ v_normals_local[fi]).astype(np.float64)
            shade = float(np.clip(0.28 + 0.65 * max(0.0, float(normal @ light_dir)), 0.0, 1.0))
