...
def parse_args() -> argparse.Namespace:
    ap = argparse.ArgumentParser(description="Export a compact drift cache (.npz) from combined_environment.nc (NetCDF4/HDF5).")
    ap.add_argument("--nc", type=str, required=True, help="Path to combined_environment.nc (NetCDF4/HDF5).")
...
def main() -> int:
    ...
    with h5py.File(nc_path, "r") as f:
        ...
        if args.u_var not in f or args.v_var not in f:
            raise KeyError(f"Missing u/v variables: {args.u_var}, {args.v_var}")
