...
def main() -> int:
    ...
    run_id = f"h1_headless_{args.task}_{stamp}_seed{int(args.seed)}_n{int(args.n_agents)}"
    out_dir = Path(args.out_dir).expanduser().resolve() if args.out_dir else (Path("runs") / "headless" / run_id).resolve()
    ...
    t0 = time.time()
    env = HeadlessOceanEnv(env_cfg, out_dir=out_dir, seed=int(args.seed), n_agents=int(args.n_agents))
    ...
    # also mirror to recorder convention
    try:
