...
def _run_one(*, python: str, repo_root: Path, spec: RunSpec, out_root: Path, dry_run: bool) -> None:
    ...
    cmd = [python, "tracks/h3_oceangym/run_task_suite.py", "--out_dir", str(out_dir)] + list(spec.args)
    env = dict(os.environ)
    ...
    print("[h3-latest] run:", spec.name)
    print("[h3-latest] out:", out_dir)
    if dry_run:
        print("[h3-latest] cmd:", " ".join(cmd))
        return
...
def main() -> int:
    ...
    out_root = Path(args.out_root) if args.out_root else Path("runs") / "oceangym_h3" / f"h3_LATEST_{_tag_now_local()}"
