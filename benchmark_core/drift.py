    ...
    def __init__(self, cache_path: str | Path) -> None:
        ...
        if "elevation" in payload.files:
            ...
            if elevation.shape != self.u.shape:
                raise ValueError(f"elevation shape mismatch: {elevation.shape} vs {self.u.shape}")
        ...
        if "land_mask" in payload.files:
            ...
            if land_mask.shape != self.u.shape:
                raise ValueError(f"land_mask shape mismatch: {land_mask.shape} vs {self.u.shape}")
...
def sample_drift_xz(position_xyz: np.ndarray, step_index: int, config: DriftConfig) -> tuple[float, float]:
    """Return a coarse current/drift vector on the simulation ground plane (x, z)."""
