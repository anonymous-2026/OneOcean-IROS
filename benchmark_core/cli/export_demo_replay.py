...
def parse_args() -> argparse.Namespace:
    ap = argparse.ArgumentParser(description="Export a benchmark-core run into demo-compatible JSON (env + paths).")
    ap.add_argument("--run-dir", type=str, required=True, help="runs/benchmark_core/... episode dir (contains agents/* and run_meta.json).")
...
def main() -> int:
    ...
    path_data = {
        ...
        "notes": {
            ...
            "coord_convention": "x,z are meters in the benchmark tile; exported y is -depth_m (so deeper => more negative).",
        },
    }
    ...
    bundle = {
        "type": "oneocean_benchmark_core_replay_bundle",
        "export_manifest": {
            "run_dir": str(run_dir),
            "out_dir": str(out_dir),
