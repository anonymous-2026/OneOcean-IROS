...
from benchmark_core.tasks import CANONICAL_TASKS_10, preset_task
...
def parse_args() -> argparse.Namespace:
    ap = argparse.ArgumentParser(description="Run a benchmark-core matrix and aggregate results.")
...
def main() -> int:
    ...
    root = Path(args.out_dir).expanduser().resolve() if args.out_dir else (Path("runs") / "benchmark_core" / f"matrix_{stamp}").resolve()
