...
def parse_args() -> argparse.Namespace:
    ap = argparse.ArgumentParser(description="Run one benchmark-core episode and record artifacts.")
...
def main() -> int:
    ...
    batch_id = f"benchmark_core_{args.task}_{stamp}_seed{int(args.seed)}_n{int(args.n_agents)}"
    base_dir = Path(args.out_dir).expanduser().resolve() if args.out_dir else (Path("runs") / "benchmark_core" / batch_id).resolve()
